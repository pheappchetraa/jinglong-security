<div>
    @if ($submitted)
        <div class="rounded-md border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-800 dark:border-emerald-900 dark:bg-emerald-900/20 dark:text-emerald-300">
            អរគុណសម្រាប់ការស្នើសុំរបស់អ្នក! ក្រុមការងាររបស់យើងនឹងទាក់ទងអ្នកឆាប់ៗនេះ ដើម្បីកំណត់ពេលវាយតម្លៃទីតាំង។<br />
            <span class="text-xs opacity-80">Thank you for your request! Our team will contact you shortly to schedule your site assessment.</span>
        </div>
    @else
        <form wire:submit="submit" class="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <div class="sm:col-span-2">
                <label class="mb-1 block text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">
                    សេវាកម្មដែលត្រូវការ / Service Needed
                </label>
                <select wire:model="service_needed"
                    class="w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 focus:border-green-800 focus:outline-none dark:border-gray-700 dark:bg-gray-800 dark:text-white">
                    <option value="">-- សូមជ្រើសរើសសេវាកម្ម / Select a service --</option>
            ...
            <div class="sm:col-span-2">
                <label class="mb-1 block text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">
                    សារបន្ថែម (មិនចាំបាច់) / Message (optional)
                </label>
            ...
            <div class="sm:col-span-2">
                <button type="submit" wire:loading.attr="disabled" wire:target="submit"
                    ...
                    <span wire:loading.remove wire:target="submit">ស្នើសុំការវាយតម្លៃឥឡូវនេះ / Request Assessment Now</span>
                    <span wire:loading wire:target="submit">កំពុងផ្ញើ... / Submitting...</span>
                </button>
            </div>
        </form>
    @endif
